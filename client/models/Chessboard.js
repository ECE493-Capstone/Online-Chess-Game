...
class Chessboard {
  constructor() {
    this._board = [ // uppercase: black, lowercase: white
      ["R", "N", "B", "Q", "K", "B", "N", "R"], // [0]
      ["P", "P", "P", "P", "P", "P", "P", "P"], // [1]
  ...
  get(row, col) { // null if no piece on the square
  ...
  move(fromRow, fromCol, toRow, toCol, promotionPiece = null) {
    ...
    if (piece.toLowerCase() === 'k' ) {
      ...
      if (Math.abs(toCol - fromCol) === 2) { // castling
    ...
    if (piece.toLowerCase() === 'r' && fromCol === 0) {
      this._castlingRights = this._castlingRights.filter(right => right !== QUEEN_SIDE_CASTLE);
  ...
  _isSameSide(piece) {
    return this._turn === WHITE ? piece === piece.toLowerCase() : piece === piece.toUpperCase();
  }
  ...
  getLegalMoves(fromRow, fromCol) {
    ...
    switch (piece.toLowerCase()) {
      case 'p':
        ...
        if (fromRow === startRow && this.get(fromRow + 2 * direction, fromCol) === null)
      ...
      case 'n':
        ...
        for (const [dx, dy] of knightMoves) {
          ...
          const newCol = fromCol + dy;
          if (newRow >= 0 && newRow <= 7 && newCol >= 0 && newCol <= 7) {
            if (this.get(newRow, newCol) === null || !this._isSameSide(this.get(newRow, newCol)))
      ...
      case 'k':
        ...
        const allAttackedSquares = this._getAllAttackedSquares();
        const isAttacked = (row, col) => allAttackedSquares.some(([r, c]) => r === row && c === col);
        for (const [dx, dy] of kingMoves) {
          const newRow = fromRow + dx;
          const newCol = fromCol + dy;
          if (newRow >= 0 && newRow <= 7 && newCol >= 0 && newCol <= 7) {
            if (isAttacked(newRow, newCol))
              continue;
        ...
        if (this._turn === WHITE) {
          if (this._castlingRights.includes(KING_SIDE_CASTLE)
            && this.get(7, 5) === null && !isAttacked(7, 5)
            && this.get(7, 6) === null && !isAttacked(7, 6)) {
            possibleMoves.push(KING_SIDE_CASTLE);
          }
          if (this._castlingRights.includes(QUEEN_SIDE_CASTLE) 
            && this.get(7, 1) === null && !isAttacked(7, 1)
            && this.get(7, 2) === null && !isAttacked(7, 2)
            && this.get(7, 3) === null && !isAttacked(7, 3)) {
            possibleMoves.push(QUEEN_SIDE_CASTLE);
          }
        } else {
          if (this._castlingRights.includes(KING_SIDE_CASTLE) 
            && this.get(0, 5) === null && !isAttacked(0, 5)
            && this.get(0, 6) === null && !isAttacked(0, 6)) {
            possibleMoves.push(KING_SIDE_CASTLE);
          }
          if (this._castlingRights.includes(QUEEN_SIDE_CASTLE) 
            && this.get(0, 1) === null && !isAttacked(0, 1)
            && this.get(0, 2) === null && !isAttacked(0, 2)
            && this.get(0, 3) === null && !isAttacked(0, 3)) {
            possibleMoves.push(QUEEN_SIDE_CASTLE);
          }
        }
        break;
  ...
  prettyPrint() {
    const prettyBoard = this._board.map(row => row.map(piece => piece === null ? '| |' : `|${piece}|`).join('')).join('\n');
